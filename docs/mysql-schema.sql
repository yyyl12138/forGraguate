-- MySQL schema for the log traceability system.
-- Stage 2 contract:
--   logtrace_node1 is the main business database.
--   logtrace_node2 and logtrace_node3 are plaintext log replica databases.
-- Local mock mode creates three schemas in one MySQL instance:
--   logtrace_node1, logtrace_node2, logtrace_node3
-- Real deployment uses the same table responsibilities on three separate MySQL servers.

CREATE DATABASE IF NOT EXISTS logtrace_node1
  DEFAULT CHARACTER SET utf8mb4
  DEFAULT COLLATE utf8mb4_bin;

CREATE DATABASE IF NOT EXISTS logtrace_node2
  DEFAULT CHARACTER SET utf8mb4
  DEFAULT COLLATE utf8mb4_bin;
...
-- Replica databases only keep plaintext logs and local batch metadata.
-- They intentionally do not contain user, audit, write-audit, or tamper procedure objects.

CREATE TABLE IF NOT EXISTS logtrace_node2.log_records LIKE logtrace_node1.log_records;
CREATE TABLE IF NOT EXISTS logtrace_node2.log_batches LIKE logtrace_node1.log_batches;

CREATE TABLE IF NOT EXISTS logtrace_node3.log_records LIKE logtrace_node1.log_records;
CREATE TABLE IF NOT EXISTS logtrace_node3.log_batches LIKE logtrace_node1.log_batches;

-- Demo-only tamper procedures.
-- These procedures intentionally exist only in logtrace_node1.
-- Do not create or enable them in logtrace_node2 or logtrace_node3.
-- Backend normal business code must not call these procedures.
-- The independent hacker simulation script calls them after it has direct
-- node1 MySQL access in the demo scenario.
